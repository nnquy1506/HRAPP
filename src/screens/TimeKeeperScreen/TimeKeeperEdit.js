...
const TimeKeeperEditScreen = ({ navigation, route }) => {
  ...
  const submittimeKeeper = (data) => {
    ...
    const params = {
      id: item.id,
      C_TenMayChamCong: data.name || "",
      C_MoTa: data.moTa || "",
      C_TrangThai: 1,
      C_Code: _.toUpper(_.snakeCase(_.deburr(data.name))),
    };
    dispatch(
      timeKeeperService.editTimeKeeper(params, onSuccessEdit, onErrorEdit)
    );
  };
    ...
    toast.show({
      render: () => (
        <AlertComponent
          ...
          title={NotiMessageSuccess.timeKeeperEdit}
        />
      ),
      placement: "top",
    });
  ...
  const onErrorDelete = () => {
    toast.show({
      render: () => (
        <AlertComponent
          status={NotiType.SUCCESS}
          title={NotiMessageSuccess.timeKeeperDelete}
        />
      ),
      placement: "top",
    });
...
export default TimeKeeperEditScreen;
