...
const JobTitleEditScreen = ({ navigation, route }) => {
  ...
  const submitJobTitle = (data) => {
    ...
    const params = {
      id: item.id,
      C_TenChucDanh: data.name || "",
      C_MoTa: data.moTa || "",
      C_TrangThai: 1,
      C_Code: _.toUpper(_.snakeCase(data)),
    };
    dispatch(jobTitleService.editJobTitle(params, onSuccessEdit, onErrorEdit));
  };
    ...
    toast.show({
      render: () => (
        <AlertComponent
          ...
          title={NotiMessageSuccess.JobTitleEdit}
        />
      ),
      placement: "top",
    });
  ...
  const onErrorDelete = () => {
    toast.show({
      render: () => (
        <AlertComponent
          status={NotiType.SUCCESS}
          title={NotiMessageSuccess.JobTitleDelete}
        />
      ),
      placement: "top",
    });
...
export default JobTitleEditScreen;
